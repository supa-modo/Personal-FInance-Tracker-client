...
const BalanceUpdatesEnhanced = ({ source }) => {
  ...
  const updatesWithChanges = sortedUpdates.map((update, index, arr) => {
    ...
    if (index < arr.length - 1) {
      const nextIndex =
        sortOrder === "desc" ? index + 1 : arr.length - 1 - index;
      const prevIndex =
        sortOrder === "desc" ? index + 1 : arr.length - 1 - index;

      if (prevIndex >= 0 && prevIndex < arr.length) {
        changeAmount =
          parseFloat(update.balance) - parseFloat(arr[prevIndex].balance);
      }
  ...
  return (
    <div className="mb-8 bg-slate-800/60 shadow-xl md:rounded-xl overflow-hidden border border-slate-700/50 backdrop-blur-[1.5px] hover:shadow-primary-900/20 hover:border-slate-600/50 transition-all duration-300">
      <div className="px-6 py-5 border-b border-slate-700/50 flex flex-col sm:flex-row sm:items-center justify-between">
        <h2 className="text-lg font-semibold text-primary-600 mb-3 sm:mb-0">
          Balance Updates
        </h2>
        ...
        {filteredUpdates.length > 0 ? (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <div
                        className={`flex-shrink-0 h-10 w-10 rounded-lg shadow-lg flex items-center justify-center ${
                      ...
                      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        <div>
                          ...
                          <div className="mt-1 text-white">
                            {formatCurrency(
                              update.balance - update.changeAmount
                            )}
                          </div>
                        </div>
          ...
          <div className="px-6 py-10 text-center">
            ...
            <p className="text-sm text-slate-500">
              {filterType !== "all"
                ? `No ${
                    filterType === "increase" ? "increases" : "decreases"
